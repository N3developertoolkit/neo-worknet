    ...
    public async Task RunAsync(uint secondsPerBlock, IConsole console, CancellationToken token)
    {
        ...
        _ = Task.Run(() =>
        {
            try
            {
                ...
                PluginHandler.LoadPlugins(Path.Combine(AppContext.BaseDirectory, "plugins"), console);
                PluginHandler.LoadPlugins(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".neo", "plugins"), console);
